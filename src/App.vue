...
const register = async (values) => {
  ...
  try {
    ...
    const existingUser = userSnapshot.docs.find((doc) => doc.data().email === newUser.email)
...
const schema = yup.object({
  ...
  terms: yup
    .bool()
    .required('Вы должны согласиться с условиями пользования и политикой конфиденциальности.'),
})
</script>
<template>
  <div class="container">
      ...
      <div class="form-group">
        ...
        <div class="custom-select">
          <Field as="select" class="form-control" id="city" name="city">
            ...
            <option v-for="city in cities" :key="city.key" :value="city.value">
              {{ city.value }}
            </option>
          </Field>
        </div>
    ...
    <br />
    <br />
    <div v-show="error" class="error">Произошла ошибка:</div>
  </div>
</template>
